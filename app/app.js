...
//var miolayer = map.getLayer('point');
      ...
      map.addLayer({
        ...
        "source": {
            "type": "vector",
            "tiles": ["https://geospatial.jrc.ec.europa.eu/geoserver/gwc/service/wmts?layer=dopa_analyst:ceap_africa_grid&tilematrixset=EPSG:900913&Service=WMTS&Request=GetTile&Version=1.0.0&Format=application/x-protobuf;type=mapbox-vector&TileMatrix=EPSG:900913:{z}&TileCol={x}&TileRow={y}"]
            },
        "source-layer": "ceap_africa_grid",
        'paint': {
...
const keyOrder = ['distance', 'distance_inv', 'powerplant', 'solar', 'wind', 'hydro', 'access', 'access_inv', 'grid', 'industrial', 'popdens', 'health', 'education', 'edu_no_e', 'irrigation', 'groundw', 'lives', 'elevation', 'slope', 'natarea', 'intactf', 'tempano', 'drought', 'conflict', 'connect'];
...
const titles = {
  ...
  elevation: "Elevation",
  slope: "Slope",
  natarea: "Natural Areas",
  intactf: "Intact Moist Forest",
...
function resetAllValues() {
  var elements = [
    ...
    'connect', 'access', 'access_inv', 'grid', 'industrial', 'popdens',
    'health', 'education', 'edu_no_e', 'irrigation', 'groundw', 'lives',
    'elevation', 'slope', 'natarea', 'intactf', 'pca', 'tempano',
...
$("#country_name").click(function(){
  ...
  $(document).ready(function() {
    const ids = [
      ...
      "access", "access_inv", "grid", "industrial", "popdens", "health", "education", 
      "edu_no_e", "irrigation", "groundw", "lives", "elevation", "slope", "natarea", 
      "intactf", "pca", "tempano", "drought", "foodsec", "conflict", "refugee"
    ];
...
// Call the function to calculate stats
  ...
  let properties = [
    ...
    'pca', 'natarea', 'intactf', 'industrial', 'drought',
    'health', 'grid', 'elevation', 'education', 'edu_no_e',
    'distance', 'distance_inv', 'conflict', 'wind', 'hydro', 'connect'
  ];
...
properties.forEach(property => {
  ...
  properties.forEach(property => {
    ...
    if (!isFinite(max_val)){
      ...
      [ "*", ['get', 'intactf'],parseInt(scores['score_intactf'])],
      [ "*", ['get', 'industrial'],parseInt(scores['score_industrial'])],
      ...
      [ "*", ['get', 'connect'],parseInt(scores['score_connect'])]
      ],
      ...
      $('.legend').empty().append("<div id='country_sel_legend'> <p class='country_sel_legend_title'>Score in Country</p>"+
        ...
        var sum = irrigation.map(function (num, idx) {
          ...
          intactf[idx]*parseInt(scores['score_intactf'])+
          industrial[idx]*parseInt(scores['score_industrial'])+ 
          ...
          connect[idx]*parseInt(scores['score_connect'])
        });
...
map.on('draw.create', function(e){
    ...
    var properties = [
      ...
      "powerplant", "popdens", "pca", "natarea", "intactf", "industrial", "drought", "health", 
...
var weights = [
  ...
  score_intactf, score_industrial, score_drought, score_health, score_grid, score_elevation, 
  score_education, score_edu_no_e, score_distance, score_distance_inv, score_conflict, 
  score_wind, score_hydro, score_connect
];
    ...
    series: [
      {
        ...
        parseInt(scores['score_intactf'])*intactfTotal,	
        parseInt(scores['score_industrial'])*industrialTotal,	
        ...
        parseInt(scores['score_connect'])*connectTotal]        
    },      
 {
      marker: {
      enabled: false,
